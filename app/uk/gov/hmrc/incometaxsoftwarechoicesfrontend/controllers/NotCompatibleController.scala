...
@Singleton
class NotCompatibleController @Inject()(view: NotCompatibleView,
  ...
  def show(): Action[AnyContent] = (identify andThen requireData) { request =>
    ...
    request.product match {
     ...
     case _ => throw new SCInconsistentDataException("[NotCompatibleController][show] - Could not find details of a recognised software product in answers")
    }
