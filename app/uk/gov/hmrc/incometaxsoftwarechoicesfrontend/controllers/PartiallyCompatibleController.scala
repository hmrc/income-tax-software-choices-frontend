...
@Singleton
class PartiallyCompatibleController @Inject()(view: PartiallyCompatibleView,
  ...
  def show(): Action[AnyContent] = (identify andThen requireData) { request =>
    ...
    request.product match {
      ...
      case _ => InternalServerError("[PartiallyCompatibleController][show] - Could not find software name and software product ID in answers]")
    }
