...
import uk.gov.hmrc.incometaxsoftwarechoicesfrontend.views.html.NotApproved

import javax.inject.{Inject, Singleton}

@Singleton
class NotApprovedController @Inject()(view: NotApproved,
  ...
  def show(): Action[AnyContent] = (identify andThen requireData) { request =>
    ...
    // just a placeholder controller/page
    Ok(view(
      postAction = routes.CheckSoftwareController.submit(),
      backLink = routes.CheckSoftwareController.show().url
    ))
  }
