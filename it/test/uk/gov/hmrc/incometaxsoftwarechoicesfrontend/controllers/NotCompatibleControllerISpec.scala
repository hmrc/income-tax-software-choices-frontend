...
class NotCompatibleControllerISpec
  extends ComponentSpecBase with BeforeAndAfterEach with DatabaseHelper {

  s"GET ${routes.NotCompatibleController.show().url}" should {
    ...
    "display the page for a recognised product" in {
      ...
      val res = SoftwareChoicesFrontend.getNotCompatible()

      res should have(
        httpStatus(OK),
        pageTitle(s"${messages("not-compatible.heading", softwareProduct.name)} - ${PageContentBase.title} - GOV.UK"),
      )
    }
