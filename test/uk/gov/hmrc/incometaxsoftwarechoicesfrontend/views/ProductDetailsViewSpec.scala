...
class ProductDetailsViewSpec extends ViewSpec with BeforeAndAfterEach {
  ...
  "ProductDetailsPage" when {
    ...
    "in personalised view when the user has only selected free with conditions" which {

      val document: Document = createAndParseDocument(softwareVendorModelFull, filtersOnlyFreeWithConditions)
      ...
      "display all tables with correct details" which {
        ...
        "displays rows only in the feature status table" in {
          checkRow(table(1), 1, readyNow, readyNowDescription)
          checkRow(table(1), 2, inDevelopment, inDevelopmentDescription)
          checkRow(table(1), 3, notIncluded, notIncludedDescription)
          thereAreNoRows(table(2))
          thereAreNoRows(table(3))
        }
      }
      ...
      "have an 'other potential software features' section" which {
        ...
        "display all tables with correct details" which {
          ...
          "displays all the rows" in {
            checkRow(detailsElementTable(1), 1, freeVersion, readyNow)
            checkRow(detailsElementTable(1), 2, agent, readyNow)
            checkRow(detailsElementTable(1), 3, individual, readyNow)
            checkRow(detailsElementTable(1), 4, standardUpdatePeriods, readyNow)
            checkRow(detailsElementTable(1), 5, calendarUpdatePeriods, readyNow)
            checkRow(detailsElementTable(1), 6, recordKeeping, readyNow)
            checkRow(detailsElementTable(1), 7, bridging, readyNow)
            checkRow(detailsElementTable(1), 8, vat, readyNow)
            checkRow(detailsElementTable(1), 9, hmrcAssist, readyNow)

            checkRow(detailsElementTable(2), 1, soleTrader, readyNow)
            checkRow(detailsElementTable(2), 2, ukProperty, readyNow)
            checkRow(detailsElementTable(2), 3, foreignProperty, readyNow)
