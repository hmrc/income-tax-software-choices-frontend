...
class PageAnswersServiceSpec extends PlaySpec with BeforeAndAfterEach {
  ...
  private val userFilterWithRandomOrder = UserFilters(sessionId, Some(fullUserAnswers), Seq.empty, Some(1234L), testTime)
  private val resetUserFilterWithRandomOrder = UserFilters(sessionId, Some(UserAnswers()), Seq.empty, Some(1234L), testTime)
    ...
    "provided with a session id" must {
      "return true to show the answers have been reset" when {
        ...
        "the userFilters do not exist" in new Setup {
          when(mockUserFiltersRepository.get(eqTo(sessionId)))
            .thenReturn(Future.successful(None))
          when(mockUserFiltersRepository.set(UserFilters(sessionId, Some(UserAnswers()), Seq.empty, None, any())))
            .thenReturn(Future.successful(true))
